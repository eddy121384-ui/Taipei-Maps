...
echo Taipei-Maps Issue #31 - z16/z17/z18 footprint identity audit
...
echo [1/3] Preparing source-ID audit GeoJSON plus footprint-size metrics...
...
echo.
echo [2/3] Building a z16-z18 source-ID audit PMTiles archive...
"%JAVA_CMD%" -Xmx2g -jar .cache\planetiler\planetiler.jar generate-custom --schema=tools\data\taipei_building_height_footprint_audit_pmtiles.yml --output=public\generated\taipei_building_height_footprint_audit.pmtiles --minzoom=16 --maxzoom=18 --render_maxzoom=18 --min_feature_size=0 --min_feature_size_at_max_zoom=0 --simplify_tolerance=0 --simplify_tolerance_at_max_zoom=0 --force
if errorlevel 1 (
  echo.
...
echo The page will scan z16, z17, and z18 and compare unique source-ID retention.
