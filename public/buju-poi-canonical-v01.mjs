...
function pairFacts(a,b){
  ...
  const ba=normalizeToken(a.branch),bb=normalizeToken(b.branch),na=normalizeToken(a.rawName),nb=normalizeToken(b.rawName),aa=normalizeAddress(a.address),ab=normalizeAddress(b.address);
  return {distance,sameBranch:Boolean(ba&&bb&&ba===bb),exactRaw:Boolean(na&&nb&&na===nb),sameAddress:Boolean(aa&&ab&&aa===ab),genericSpecific:Boolean(a.generic!==b.generic)};
}
function mergeDecision(a,b){
  ...
  if(e.sameAddress&&e.distance<=max)return {...e,merge:true,reason:'same-address',rank:5};
  if(e.sameBranch&&e.distance<=max)return {...e,merge:true,reason:'same-branch',rank:5};
