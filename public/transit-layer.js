(()=>{
  ...
  const TRA_DASH=[1.4,1.0];
  const THSR_DASH=[3.4,1.4];
  ...
  class TransitLayer{
    ...
    applyRegionalAppearance(inTaiwan,inNorth=centerInsideNorthTaiwan(this.map)){
      ...
      if(this.map.getLayer(LAYERS.tra)){
        this.map.setPaintProperty(LAYERS.tra,'line-color',inTaiwan?TAIWAN_TRA_COLOR:GLOBAL_RAIL_COLOR);
        this.map.setPaintProperty(LAYERS.tra,'line-dasharray',inTaiwan?TRA_DASH:null);
      }
    }
