(()=>{
  ...
  const TAIWAN_ONLY_LAYER_IDS=[...TAIPEI_LINE_LAYER_IDS,...TAIPEI_STATION_LAYER_IDS,...NORTH_URBAN_LINE_LAYER_IDS,...NORTH_URBAN_STATION_LAYER_IDS];
  ...
  function lineLayer(id,filter,color,isCasing=false,opacity=null){
    return {id,type:'line',source:SOURCE_ID,'source-layer':SOURCE_LAYER,minzoom:8,filter,layout:{visibility:'none','line-cap':'round','line-join':'round'},paint:{'line-color':color,'line-width':isCasing?casingWidth:width,'line-opacity':opacity??(isCasing?.88:.96)}};
  }
  function localRailLineLayer(id,source,isCasing=false){
    return {id,type:'line',source,minzoom:9,layout:{visibility:'none','line-cap':'round','line-join':'round'},paint:{'line-color':isCasing?'rgba(255,255,255,.97)':['get','line_color'],'line-width':isCasing?localCasingWidth:localWidth,'line-opacity':isCasing?.94:.99}};
  }
  function stationPointLayer(id,source){
    return {id,type:'circle',source,minzoom:10.5,layout:{visibility:'none'},paint:{'circle-radius':['interpolate',['linear'],['zoom'],10.5,2.5,13,3.4,16,4.6],'circle-color':'rgba(255,255,255,.99)','circle-opacity':.99,'circle-stroke-color':'#263238','circle-stroke-width':['interpolate',['linear'],['zoom'],10.5,1.0,14,1.35,17,1.7],'circle-stroke-opacity':.94}};
  }
  function stationLabelLayer(id,source){
    return {id,type:'symbol',source,minzoom:11.6,layout:{visibility:'none','text-field':['get','station_name'],'text-size':['interpolate',['linear'],['zoom'],11.6,10.5,14,11.5,17,13],'text-offset':[0,1.0],'text-anchor':'top','text-max-width':8,'text-allow-overlap':false,'text-ignore-placement':false},paint:{'text-color':'#263238','text-halo-color':'rgba(255,255,255,.97)','text-halo-width':1.35,'text-halo-blur':.25}};
  }
  ...
  class TransitLayer{
    ...
    async init(){
      ...
      try{
        ...
        const mrtFilter=railFilter(['subway','monorail','light_rail']),traFilter=railFilter(['narrow_gauge']),thsrFilter=railFilter(['standard_gauge']);const layers=[lineLayer(LAYERS.mrtCasing,mrtFilter,'rgba(255,255,255,.96)',true,.88),lineLayer(LAYERS.mrt,mrtFilter,GLOBAL_METRO_COLOR,false,.96),lineLayer(LAYERS.traCasing,traFilter,'rgba(255,255,255,.96)',true),lineLayer(LAYERS.tra,traFilter,GLOBAL_RAIL_COLOR),lineLayer(LAYERS.thsrCasing,thsrFilter,'rgba(255,255,255,.96)',true),lineLayer(LAYERS.thsr,thsrFilter,GLOBAL_RAIL_COLOR)];const beforeId=this.map.getLayer('building')?'building':undefined;for(const layer of layers)if(!this.map.getLayer(layer.id))this.map.addLayer(layer,beforeId);
