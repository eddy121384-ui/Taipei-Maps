(()=>{
  ...
  function lineLayer(id,filter,color,isCasing=false,opacity=null){return {id,type:'line',source:SOURCE_ID,'source-layer':SOURCE_LAYER,minzoom:8,filter,layout:{visibility:'none','line-cap':'round','line-join':'round'},paint:{'line-color':color,'line-width':isCasing?casingWidth:width,'line-opacity':opacity??(isCasing?.88:.96)}};}
  function localRailLineLayer(id,source,isCasing=false,minzoom=9){return {id,type:'line',source,minzoom,layout:{visibility:'none','line-cap':'round','line-join':'round'},paint:{'line-color':isCasing?'rgba(255,255,255,.97)':['get','line_color'],'line-width':isCasing?localCasingWidth:localWidth,'line-opacity':isCasing?.94:.99}};}
  ...
  class TransitLayer{
    ...
    async init(){
      ...
      try{
        ...
        try{const count=await this.loadIntercityThsr(beforeId);console.info(`Taiwan THSR local geometry ready: ${count} feature(s)`);}catch(error){this.intercityThsrReady=false;console.warn('Local THSR geometry unavailable; keeping generic green rail base',error);}
