(()=>{
  ...
  const looksLikeListing=text=>{
    ...
    const price=/[0-9][0-9,]*(?:\.[0-9]+)?\s*萬/.test(t);
    const detail=/(坪|房|屋齡|樓層|公寓|大樓|華廈|電梯)/.test(t);
    return price&&detail;
  };
  const score=el=>{
    ...
    if(/屋齡|樓層|公寓|大樓|華廈|電梯/.test(t))s+=1;
  ...
  const bestHref=el=>{
    ...
    try{const u=new URL(a.href,location.href);return u.origin===location.origin?u.href:location.href}catch{return location.href}
  };
  ...
  function scanVisible(){
    const raw=[...document.querySelectorAll('article,li,a[href],div')]
      .filter(isVisible)
      .filter(el=>looksLikeListing(el.innerText))
      .map(el=>({el,text:clean(el.innerText),score:score(el),area:el.getBoundingClientRect().width*el.getBoundingClientRect().height}))
      .filter(x=>x.score>=6)
    ...
    const seen=new Set();
    for(const x of raw){
      const key=x.text.slice(0,500);
      if(seen.has(key))continue;
      if(chosen.some(y=>y.el.contains(x.el)&&y.text===x.text))continue;
      chosen.push(x);seen.add(key);
      if(chosen.length>=80)break;
    }
    return chosen.map((x,i)=>captureCandidate(x.el,i));
  }
